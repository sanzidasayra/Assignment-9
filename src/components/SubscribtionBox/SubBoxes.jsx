...
const SubBoxes = ({ boxes = [] }) => {
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-20 py-10">
      <div className=" bg-blue-50 dark:bg-gray-900 shadow-lg dark:shadow-xl rounded-3xl mt-10 p-6 md:p-8 lg:p-10">
        ...
        <p className="text-sm md:text-base text-center mt-4 lg:mt-6 max-w-4xl mx-auto text-gray-700 dark:text-gray-300">
          Explore our curated winter subscription boxes, each filled with
          handpicked seasonal items to bring warmth, joy, and festive cheer.
          Whether you crave comfort, adventure, or holiday magic, there’s a box
          crafted just for you. Unbox happiness and make your winter moments
          truly special.
        </p>
      </div>
      ...
      {loading ? (
        <div className="flex justify-center items-center py-10">
          <div className="spinner-border animate-spin inline-block w-12 h-12 border-4 border-solid rounded-full border-blue-500 border-t-transparent"></div>
        </div>
      ) : (
        <div className="grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 bg-blue-50 dark:bg-gray-900 shadow-lg dark:shadow-xl rounded-3xl mt-10 p-6 md:p-8 lg:p-10">
          ...
          >
            {visibleSubBoxes.map((singleSubscriptionBox) => (
              <SubBox
                key={singleSubscriptionBox.id}
                singleSubscriptionBox={singleSubscriptionBox}
              />
            ))}
          </Suspense>
        </div>
